...
async function updateSession(
  ...
  const matchedPaths: string[] = middlewareAuth.unauthenticatedPaths.filter(
    (pathGlob: string) => {
  ...
  if (middlewareAuth.enabled && matchedPaths.length === 0 && !session) {
    if (debug)
      console.log(
        `Unauthenticated user on protected route ${request.url}, redirecting to AuthKit`,
      )
  ...
  try {
    if (debug)
      console.log(
        `Session invalid. Refreshing access token that ends in ${session.accessToken.slice(-10)}`,
      )
    ...
    if (debug)
      console.log(
        `Refresh successful. New access token ends in ${accessToken.slice(-10)}`,
      )
...
function getMiddlewareAuthPathRegex(pathGlob: string) {
  ...
  try {
    ...
    throw new Error(
      `Error parsing routes for middleware auth. Reason: ${message}`,
    )
  }
}
